...
from poker_env.agents.baseline_agent.LimitTexas_baseline_agent import LimitTexasTPAgent, LimitTexasLPAgent, LimitTexasLAAgent, LimitTexasTAAgent
from poker_env.LimitTexas.action import Action as LimitTexasAction
from poker_env.agents.lbr_agent.opponent_modeling import Classifier
from poker_env.LimitTexas.env import LimitTexasHoldemEnv
from poker_env.dependency.pokerstove.utils import get_data

class LimitTexasLBRAgent(Agent):
    '''
    ...
    def __init__(self, mode, agent_id, agent=None, player_num=2, training_times=10, agent_name='random'):
        ...
        self.env = LimitTexasHoldemEnv(num_players=player_num, allow_step_back=True)
        self.classifier = Classifier(self.opp_id)
        for action in LimitTexasAction:
    ...
    def eval_step(self, state):
        ...
        if max(action_utilities) <= 0:
            action = LimitTexasAction.FOLD.value
        else:
    ...
    def get_value(self, state, env):
        ...
        for a in legal_actions:
            ...
            if not is_end:
                if str_action == LimitTexasAction.CALL.value:
                    action_utilities[a] = wp * pot - (1 - wp) * asked
                elif str_action == LimitTexasAction.FOLD.value:
                    continue
                else:
    ...
    def opponent_modeling(self, trajectories_list):
        ...
        if self.opponent_name == 'TP':
            self.opponent = LimitTexasTPAgent()
        elif self.opponent_name == 'LP':
            self.opponent = LimitTexasLPAgent()
        elif self.opponent_name == 'LA':
            self.opponent = LimitTexasLAAgent()
        elif self.opponent_name == 'TA':
            self.opponent = LimitTexasTAAgent()
        elif self.opponent_name == 'INIT':
