...
class ToyPokerDLCFRPlusAgent(ToyPokerBaseBRAgent):
    '''
    ...
    def __init__(self, agent_id, max_depth=6, mode='agent', agent=None, agent_name='random', player_num=2, training_times=1, infoset_mode='all_action'):
        '''
        ...
        self.iterations = 0
        super().__init__(mode, agent_id, agent=agent, player_num=player_num, agent_name=agent_name, agent_type='DLCFRPlusAgent')
    ...
    def parallel_traverse(self, state):
        ...
        for cards in self.all_cards:
            ...
            parameters.append([cards, deepcopy(self.env), deepcopy(state)])
            self.env.shuffle_deck()
            self.env.give_back_cards(player_id=self.opp_id)

        print(len(parameters))
        process = Pool()
        process.map(self.traverse, parameters)
        process.close()
        process.join()
        return

    def traverse(self, parameters):
        cards, env, state = parameters
        legal_actions = self.encode_action(state)
        ...
        for action in legal_actions:
            ...
            state_value = self.get_value(next_state, env, 1)
    ...
    def get_value(self, state, env, depth):
        '''
